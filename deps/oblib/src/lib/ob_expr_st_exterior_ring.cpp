...
ObExprSTExteriorRing::ObExprSTExteriorRing(ObIAllocator &alloc)
    : ObFuncExprOperator(alloc, T_FUN_SYS_ST_EXTERIORRING, N_ST_EXTERIORRING, 1, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
}

ObExprSTExteriorRing::~ObExprSTExteriorRing()
{
}

int ObExprSTExteriorRing::calc_result_type1(ObExprResType &type,
                                    ObExprResType &type1,
                                    common::ObExprTypeCtx &type_ctx) const
{
  ...
  if (ob_is_numeric_type(type1.get_type())) {
    ...
    LOG_USER_ERROR(OB_ERR_GIS_INVALID_DATA, N_ST_EXTERIORRING);
...
int ObExprSTExteriorRing::eval_st_area(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(gis_arg->eval(ctx, gis_datum))) {
    ...
    if (OB_FAIL(ObTextStringHelper::read_real_string_data(temp_allocator, *gis_datum,
    ...
    } else if (OB_FAIL(ObGeoExprUtils::get_srs_item(ctx, srs_guard, wkb, srs, true, N_ST_EXTERIORRING))) {
      LOG_WARN("fail to get srs item", K(ret), K(wkb));
    } else if (OB_FAIL(ObGeoExprUtils::build_geometry(temp_allocator, wkb, geo, srs, N_ST_EXTERIORRING))) {
      ...
      LOG_USER_ERROR(OB_ERR_UNEXPECTED_GEOMETRY_TYPE, "POLYGON/MULTIPOLYGON",
        ObGeoTypeUtil::get_geo_name_by_type(geo->type()), N_ST_EXTERIORRING);
    } else {
      ...
      if (OB_FAIL(gis_context.append_geo_arg(geo))) {
        ...
        ObGeoExprUtils::geo_func_error_handle(ret, N_ST_EXTERIORRING);
...
int ObExprSTExteriorRing::cg_expr(ObExprCGCtx &expr_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
