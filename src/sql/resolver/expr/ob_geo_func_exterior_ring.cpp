...
class ObGeoFuncExteriorRingImpl : public ObIGeoDispatcher<double, ObGeoFuncExteriorRingImpl>
{
public:
  ObGeoFuncExteriorRingImpl();
  virtual ~ObGeoFuncExteriorRingImpl() = default;
...
OB_GEO_UNARY_FUNC_BEGIN(ObGeoFuncExteriorRingImpl, ObWkbGeomPolygon, double)
{
  UNUSED(context);
  INIT_SUCC(ret);
...
OB_GEO_UNARY_FUNC_BEGIN(ObGeoFuncExteriorRingImpl, ObWkbGeomMultiPolygon, double)
{
  UNUSED(context);
  INIT_SUCC(ret);
...
// geography
OB_GEO_UNARY_FUNC_BEGIN(ObGeoFuncExteriorRingImpl, ObWkbGeogPolygon, double)
{
  INIT_SUCC(ret);
...
OB_GEO_UNARY_FUNC_BEGIN(ObGeoFuncExteriorRingImpl, ObWkbGeogMultiPolygon, double)
{
  INIT_SUCC(ret);
  const ObSrsItem *srs = context.get_srs();
  if (OB_ISNULL(srs)) {
...
int ObGeoFuncExteriorRing::eval(const ObGeoEvalCtx &gis_context, double &result)
{
